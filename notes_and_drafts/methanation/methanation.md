...
We can encode the stoichiometric coefficients as a vector \(\boldsymbol{\nu}\) like so:
...
To summarize these textually: The change in concentration of each of the reactants over time is directly equal to the rate of the reaction multiplied by the stoichiometric vector \(\boldsymbol{\nu}\). 
